# newpcaps_actor_agent.py implements PCAPS (Precedence- and Carbon-Aware Provisioning and Scheduling) on top of Decima.
#
# The original PCAPS scheduling policy is preserved. Executor accounting is split into:
# - registered executors: currently available for execution;
# - allocated executors: registered + moving + committed, used for provisioning accounting.
...
class PCAPSAgent(Agent):
    def __init__(
        ...
        self.adv_loss = tf.reduce_sum(
            input_tensor=tf.multiply(
                tf.math.log(
                    self.selected_node_prob * self.selected_job_prob + self.eps
                ),
                -self.adv,
            )
        )
    ...
    def actor_network(
        ...
        gsn_summ_backward_map_extend = tf.tile(
            tf.expand_dims(gsn_summ_backward_map, axis=0),
            [batch_size, 1, 1],
        )
    ...
    @staticmethod
    def _real_jobs(job_dags):
        return [job for job in job_dags if job.name != "dummy"]

    @staticmethod
    def _count_registered_by_job(real_jobs):
        return {job: len(job.executors) for job in real_jobs}

    @staticmethod
    def _count_moving_by_job(real_jobs, moving_executors):
        moving_by_job = {job: 0 for job in real_jobs}
        for node in moving_executors.moving_executors.values():
            if node is not None and node.job_dag in moving_by_job:
                moving_by_job[node.job_dag] += 1
        return moving_by_job

    @staticmethod
    def _count_committed_by_job(real_jobs, exec_commit):
        committed_by_job = {job: 0 for job in real_jobs}
    ...
    def get_allocated_by_job(self, job_dags, exec_commit, moving_executors):
        """Build a fresh executor snapshot from the environment observation."""
        real_jobs = self._real_jobs(job_dags)
        registered = self._count_registered_by_job(real_jobs)
        moving = self._count_moving_by_job(real_jobs, moving_executors)
        committed = self._count_committed_by_job(real_jobs, exec_commit)

        return {
            job: registered[job] + moving[job] + committed[job]
    ...
    def translate_state(self, obs):
        """Translate state using an observation-derived executor snapshot."""
        (
            job_dags,
            source_job,
            num_source_exec,
        ...
        real_jobs = self._real_jobs(job_dags)
        allocated_by_job = self.get_allocated_by_job(
            job_dags,
            exec_commit,
            moving_executors,
        )

        total_num_nodes = int(sum(job.num_nodes for job in real_jobs))
        node_inputs = np.zeros([total_num_nodes, self.node_input_dim])
        job_inputs = np.zeros([len(real_jobs), self.job_input_dim])
        job_idx_by_dag = {job: idx for idx, job in enumerate(real_jobs)}
    ...
    def get_valid_masks(
        ...
        for node in frontier_nodes:
            if node.job_dag in job_valid and job_valid[node.job_dag]:
                node_valid_mask[0, action_map.inverse_map[node]] = 1
    ...
    def get_carbon_intensity(self, current_time):
        ...
        if len(future_keys) > 1:
            ...
            return current_carbon, min(future_carbon) * 0.95, max(future_carbon)

        return current_carbon, 280, 400

    def threshold(self, rel_imp):
        convex_comb = self.gamma * self.L + (1.0 - self.gamma) * self.U
        denominator = np.exp(self.gamma) - 1.0
        if abs(denominator) < self.eps:
            return convex_comb

        exp_term = (
            (self.U - convex_comb)
            * ((np.exp(self.gamma * rel_imp) - 1.0) / denominator)
        )
    ...
    def get_action(self, obs):
        ...
        valid_probs = node_act_probs[0, node_valid_mask[0, :] == 1]
        max_act_prob = max(float(np.max(valid_probs)), self.eps)
        ...
        assert job_valid_mask[
            0,
            job_act[0, job_idx] + len(self.executor_levels) * job_idx,
        ] == 1
        ...
        self.L = L
        self.U = U
        carbon_threshold = self.threshold(rel_imp)

        if carbon_threshold < current_carbon:
            self.num_deferred += 1
        ...
        if self.gamma > 0.0:
            scaled = use_exec_init * min(
                np.exp(self.gamma * (self.L - current_carbon)),
                1 - min(0.9, self.gamma),
            )
            if np.isnan(scaled):
                use_exec = use_exec_init
            else:
                diff = use_exec_init - min(
                    int(np.ceil(scaled)),
                    use_exec_init,
                )
        ...
        use_exec = max(0, min(int(use_exec), use_exec_init))
        if use_exec < 1:
            return None, num_source_exec, False
